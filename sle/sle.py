...
def cut_matrix(matrix: list, stripe: int) -> list:
    """Обрезка матрицы"""
    a = np.shape(matrix)
    ...
    if len(a) > 1:
        if stripe != 0:
            ...
            matrix_result = zero_matrix(length, length + 1)
...
def filling_matrix_cut(matrix, matrix_cut):
    if len(matrix_cut) == 1:
